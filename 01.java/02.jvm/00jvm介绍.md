...
新生代一般采用复制算法，老年代采用标记清楚和标记整理。

![](https://youcai922.github.io/99.src/img/分代回收机制.png)

jvm根据对象存货周期的不同，把内存分为几块。一般是把对象分为新生代和老年代。

先把堆按照1：2分成两部分，1/3作为新生代，2/3作为老年代。然后将新生代分成1：1：8分成10份，8/10作为Eden，另外两份叫做from survivor（s0），to survivor（S1）。

当Eden区满了之后，根据Eden和S0的区域执行复制算法，放到S1，然后交换S0和S1的名字，并将其年龄加1。

反复循环，如果S1满了就放到老年代。如果存在对象年龄达到指定年限，则存入到老年代中。当老年代区满了之后，执行标记清楚/标记整理算法。

新生代的特点就是：朝生夕死，存货率低，而S0和S1存在的目的就是让对象没有那么容易进入老年代。
